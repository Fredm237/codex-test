...
import { buildOutfitRecommendation, isOutfitStudioEnabled, type OutfitPiece, type OutfitRecommendation } from "@/lib/filon-intelligence";
import { buildCompleteRecommendation, type CompleteRecommendation, type OutfitStrategyId, type OwnedPiece } from "@/lib/filon-complete";
...
import { explanationForConfidence, type RecreateAnalysis, type RecreateObservation } from "@/lib/recreate-contract";
...
import type { OutfitSolution } from "@/lib/filon-intelligence";
import { buildDecisionLedger } from "@/lib/filon-decision-ledger";
import { selectDiscoverCapsules, type DiscoverCapsule } from "@/lib/filon-discover-capsules";
...
import { optimizeSavedOutfit, type OutfitOptimization } from "@/lib/filon-outfit-optimize";
...
const copy = {
  fr: { back: "Retour", eyebrow: "FILON Intelligence · Fashion Expert", title: "Décrivez la tenue qui vous ferait dire : c’est exactement ça.", placeholder: "Ex. un mariage civil, élégant mais simple", occasion: "Occasion", season: "Saison", budget: "Budget total", style: "Votre direction", create: "Créer une solution", loading: "FILON vérifie les offres partenaires…", noResult: "Décrivez une intention, puis FILON cherchera des pièces réellement disponibles.", reset: "Réinitialiser", minimal: "Minimal", classic: "Classique", bold: "Audacieux", wedding: "Mariage", work: "Travail", evening: "Soirée", spring: "Printemps", summer: "Été", autumn: "Automne", winter: "Hiver", solution: "Proposition vérifiable", total: "Total des pièces", styleScore: "Cohérence", confidence: "Confiance", verified: "disponible", piece: "Voir l’offre", unavailable: "Outfit Studio est désactivé dans cette version de FILON.", error: "Le catalogue est momentanément indisponible. Aucune recommandation n’a été fabriquée.", constraints: "Contraintes appliquées", why: "Pourquoi cette pièce ?", abstain: "FILON s’abstient", scoreNote: "Méthode de score", critique: "Relecture FILON", relations: "Relations analysées", feedback: "Cette proposition vous aide ?", helpful: "Oui, utile", review: "À revoir", thanks: "Votre retour est enregistré localement pour affiner vos prochains briefs.", finding: { MISSING_STRUCTURE: "Une pièce de structure renforcerait cette occasion.", MISSING_ACCESSORY: "Un accessoire pourrait compléter l’ensemble.", CONTEXT_UNSPECIFIED: "Précisez l’occasion pour calibrer davantage la proposition.", SEASON_UNSPECIFIED: "Précisez la saison pour mieux adapter les pièces.", LOW_RELATION_COVERAGE: "Les relations entre pièces restent partielles." }, role: { base: "Base", structure: "Structure", footwear: "Chaussures", accessory: "Accessoire" } },
  nl: { back: "Terug", eyebrow: "FILON Intelligence · Fashion Expert", title: "Beschrijf de outfit die precies goed voelt.", placeholder: "Bijv. een eenvoudige, elegante burgerlijke bruiloft", occasion: "Gelegenheid", season: "Seizoen", budget: "Totaalbudget", style: "Jouw richting", create: "Een oplossing maken", loading: "FILON controleert partneraanbiedingen…", noResult: "Beschrijf een intentie. FILON zoekt dan werkelijk beschikbare items.", reset: "Reset", minimal: "Minimal", classic: "Klassiek", bold: "Gedurfd", wedding: "Huwelijk", work: "Werk", evening: "Avond", spring: "Lente", summer: "Zomer", autumn: "Herfst", winter: "Winter", solution: "Controleerbaar voorstel", total: "Totaal van items", styleScore: "Samenhang", confidence: "Vertrouwen", verified: "beschikbaar", piece: "Bekijk aanbieding", unavailable: "Outfit Studio is uitgeschakeld in deze FILON-versie.", error: "De catalogus is tijdelijk niet beschikbaar. Er is geen aanbeveling gemaakt.", constraints: "Toegepaste beperkingen", why: "Waarom dit item?", abstain: "FILON onthoudt zich", scoreNote: "Scoremethode", critique: "FILON-controle", relations: "Geanalyseerde relaties", feedback: "Helpt dit voorstel u?", helpful: "Ja, nuttig", review: "Herzien", thanks: "Uw feedback wordt lokaal opgeslagen om volgende briefs te verfijnen.", finding: { MISSING_STRUCTURE: "Een structurerend item zou bij deze gelegenheid passen.", MISSING_ACCESSORY: "Een accessoire kan de look aanvullen.", CONTEXT_UNSPECIFIED: "Geef de gelegenheid op voor een betere afstemming.", SEASON_UNSPECIFIED: "Geef het seizoen op om items beter aan te passen.", LOW_RELATION_COVERAGE: "De relaties tussen items blijven gedeeltelijk." }, role: { base: "Basis", structure: "Structuur", footwear: "Schoenen", accessory: "Accessoire" } },
  en: { back: "Back", eyebrow: "FILON Intelligence · Fashion Expert", title: "Describe the outfit that would feel exactly right.", placeholder: "E.g. elegant but simple civil wedding", occasion: "Occasion", season: "Season", budget: "Total budget", style: "Your direction", create: "Create a solution", loading: "FILON is checking partner offers…", noResult: "Describe an intent and FILON will look for actually available pieces.", reset: "Reset", minimal: "Minimal", classic: "Classic", bold: "Bold", wedding: "Wedding", work: "Work", evening: "Evening", spring: "Spring", summer: "Summer", autumn: "Autumn", winter: "Winter", solution: "Verifiable proposal", total: "Pieces total", styleScore: "Coherence", confidence: "Confidence", verified: "available", piece: "View offer", unavailable: "Outfit Studio is disabled in this FILON version.", error: "The catalogue is temporarily unavailable. No recommendation was fabricated.", constraints: "Applied constraints", why: "Why this piece?", abstain: "FILON abstains", scoreNote: "Scoring method", critique: "FILON review", relations: "Analysed relations", feedback: "Does this proposal help?", helpful: "Yes, useful", review: "Needs review", thanks: "Your feedback is stored locally to refine your next briefs.", finding: { MISSING_STRUCTURE: "A structured layer would reinforce this occasion.", MISSING_ACCESSORY: "An accessory could complete the set.", CONTEXT_UNSPECIFIED: "Specify the occasion to calibrate the proposal further.", SEASON_UNSPECIFIED: "Specify the season to adapt the pieces better.", LOW_RELATION_COVERAGE: "The relations between pieces remain partial." }, role: { base: "Base", structure: "Structure", footwear: "Footwear", accessory: "Accessory" } },
};
...
const journalCopy = {
  fr: { title: "Journal de tenues", save: "Sauvegarder cette tenue", saved: "Tenue sauvegardée", empty: "Vos propositions sauvegardées apparaîtront ici.", remove: "Retirer", create: "Créée", complete: "Complétée" },
  nl: { title: "Outfitdagboek", save: "Deze outfit bewaren", saved: "Outfit bewaard", empty: "Uw bewaarde voorstellen verschijnen hier.", remove: "Verwijderen", create: "Gemaakt", complete: "Aangevuld" },
  en: { title: "Outfit journal", save: "Save this outfit", saved: "Outfit saved", empty: "Your saved proposals will appear here.", remove: "Remove", create: "Created", complete: "Completed" },
};

const ledgerCopy = {
  fr: { title: "Registre de décision", reviewed: "offres examinées", eligible: "éligibles", unavailable: "indisponibles", unsafe: "liens écartés", constraints: "Contraintes", policy: "Règles de décision" },
  nl: { title: "Beslisregister", reviewed: "bekeken aanbiedingen", eligible: "geschikt", unavailable: "niet beschikbaar", unsafe: "uitgesloten links", constraints: "Beperkingen", policy: "Beslisregels" },
  en: { title: "Decision ledger", reviewed: "offers reviewed", eligible: "eligible", unavailable: "unavailable", unsafe: "links excluded", constraints: "Constraints", policy: "Decision rules" },
};

const capsuleCopy = {
  fr: { title: "Capsules Discover", use: "Essayer", note: "Ces capsules sont des briefs de départ ; FILON vérifie ensuite les offres réellement disponibles." },
  nl: { title: "Discover-capsules", use: "Proberen", note: "Deze capsules zijn startbriefs; FILON controleert daarna werkelijk beschikbare aanbiedingen." },
  en: { title: "Discover capsules", use: "Try", note: "These capsules are starting briefs; FILON then checks actually available offers." },
};
...
const comparisonCopy = {
  fr: { title: "Comparer les stratégies", total: "Total", confidence: "Confiance", coverage: "Pièces", difference: "Écart Signature vs Safe" },
  nl: { title: "Strategieën vergelijken", total: "Totaal", confidence: "Vertrouwen", coverage: "Items", difference: "Signature versus Safe" },
  en: { title: "Compare strategies", total: "Total", confidence: "Confidence", coverage: "Pieces", difference: "Signature versus Safe" },
};
...
const optimizeCopy = {
  fr: { title: "Optimiser une tenue", action: "Optimiser", empty: "Sauvegardez une tenue pour vérifier des alternatives actuelles.", loading: "FILON vérifie les offres actuelles…", saving: "Économie observée", replace: "Remplacement vérifiable", unavailable: "Aucune amélioration vérifiable n’est disponible pour cette tenue." },
  nl: { title: "Een outfit optimaliseren", action: "Optimaliseren", empty: "Bewaar een outfit om actuele alternatieven te controleren.", loading: "FILON controleert de actuele aanbiedingen…", saving: "Vastgestelde besparing", replace: "Verifieerbare vervanging", unavailable: "Voor deze outfit is geen verifieerbare verbetering beschikbaar." },
  en: { title: "Optimise an outfit", action: "Optimise", empty: "Save an outfit to check current alternatives.", loading: "FILON is checking current offers…", saving: "Observed saving", replace: "Verifiable replacement", unavailable: "No verifiable improvement is available for this outfit." },
};

const qualityCopy = {
  fr: { title: "Signaler une correction", prompt: "Ce retour reste local et ne modifie jamais le catalogue partenaire.", save: "Enregistrer la correction", saved: "Correction enregistrée localement.", note: "Précisez si nécessaire", style: "Style", context: "Contexte", confidence: "Confiance", hallucination: "Information à vérifier" },
  nl: { title: "Een correctie melden", prompt: "Deze feedback blijft lokaal en wijzigt de partnercatalogus nooit.", save: "Correctie opslaan", saved: "Correctie lokaal opgeslagen.", note: "Voeg indien nodig details toe", style: "Stijl", context: "Context", confidence: "Vertrouwen", hallucination: "Te verifiëren informatie" },
  en: { title: "Report a correction", prompt: "This feedback stays local and never changes the partner catalogue.", save: "Save correction", saved: "Correction saved locally.", note: "Add details if needed", style: "Style", context: "Context", confidence: "Confidence", hallucination: "Information to verify" },
};
...
export default function OutfitStudioScreen() {
  ...
  const analyzeInspiration = async () => { if (!inspirationUrl.trim() || recreateMutation.isPending) return; setError(null); setRecommendation(null); setCompleteRecommendation(null); setRecreateAnalysis(null); try { setRecreateAnalysis(await recreateMutation.mutateAsync({ imageUrl: inspirationUrl.trim() })); } catch { setError(recreateText.error); } };
  ...
  const useInspirationForSearch = () => { if (!recreateAnalysis) return; const labels = [...recreateAnalysis.visiblePieces, ...recreateAnalysis.silhouette].filter((item) => item.confidence !== "unknown").slice(0, 5).map((item) => item.label); setRequest(labels.join(" ")); setMode("create"); };
  ...
  const optimizeOutfit = async (item: SavedOutfit) => {
    ...
    try {
      ...
      setOptimization({ status: "abstain", sourceOutfitId: item.id, checkedOffers: 0, reason: optimizeText.unavailable });
    } finally {
      setOptimizingOutfitId(null);
    }
  };
  const applyCapsule = (capsule: DiscoverCapsule) => { setRequest(capsule.startQuery); setOccasion(capsule.occasion); setMode("create"); };
  ...
  const submit = async () => {
    ...
    try {
      ...
      const intent = { request: request.trim(), occasion: occasion ? text[occasion] : null, season: season ? text[season] : null, budget: parsedBudget, declaredStyle: preferences.declaredStyle ? text[preferences.declaredStyle] : null };
      if (mode === "complete") {
        setSelectedStrategy("safe");
        setCompleteRecommendation(buildCompleteRecommendation(intent, { label: ownedLabel.trim(), role: ownedRole }, response.items));
      } else {
        setRecommendation(buildOutfitRecommendation(intent, response.items));
      }
    } catch {
      setError(text.error);
    } finally {
  ...
  if (!isOutfitStudioEnabled()) {
    return <ScreenContainer className="p-5" edges={["top", "left", "right", "bottom"]}><View style={styles.disabled}><MaterialIcons name="lock-outline" size={25} color={colors.primary} /><Text style={styles.disabledTitle}>Outfit Studio</Text><Text style={styles.disabledBody}>{text.unavailable}</Text><TactileButton accessibilityLabel={text.back} onPress={() => router.back()} style={styles.primaryButton}><Text style={styles.primaryButtonText}>{text.back}</Text></TactileButton></View></ScreenContainer>;
  }

  const activeCompleteStrategy = completeRecommendation?.status === "solution" ? completeRecommendation.strategies.find((strategy) => strategy.id === selectedStrategy) ?? completeRecommendation.strategies[0] : null;
  const pieces = recommendation?.status === "solution" ? recommendation.solution.pieces : activeCompleteStrategy?.solution.pieces ?? [];
  return (
    <ScreenContainer className="" containerClassName="bg-background" edges={["top", "left", "right", "bottom"]}>
      <FlatList
        ...
        ListHeaderComponent={<>
          <View style={styles.topbar}><TactileButton accessibilityLabel={text.back} onPress={() => router.back()} style={styles.back}><MaterialIcons name="arrow-back" size={20} color={colors.foreground} /><Text style={styles.backText}>{text.back}</Text></TactileButton><Pressable accessibilityRole="button" accessibilityLabel="Lookbook" onPress={() => router.push("/lookbook" as never)} style={({ pressed }) => [styles.lookbookButton, pressed && styles.pressed]}><MaterialIcons name="collections-bookmark" size={17} color={colors.primary} /><Text style={styles.lookbookText}>Lookbook</Text></Pressable><View style={styles.live}><View style={styles.liveDot} /><Text style={styles.liveText}>LIVE</Text></View></View>
          ...
          <OptimizePanel items={savedOutfits} result={optimization} loadingId={optimizingOutfitId} locale={locale} text={optimizeText} styles={styles} onOptimize={optimizeOutfit} />
          <RotationPanel suggestions={buildOutfitRotation(savedOutfits)} text={rotationText} styles={styles} onPlan={(suggestion) => beginPlanning(suggestion.outfit)} />
          ...
          {error ? <StatusCard title={text.error} body="" icon="cloud-off" styles={styles} error /> : null}
          {recommendation?.status === "abstain" ? <StatusCard title={text.abstain} body={recommendation.reason} icon="verified-user" styles={styles} error /> : null}
          {recommendation?.status === "solution" ? <><SolutionHeader recommendation={recommendation} text={text} locale={locale} styles={styles} feedback={feedback} onFeedback={recordFeedback} onSave={(solution) => void saveSolution(request, "create", solution)} saveLabel={journalText.save} /><CorrectionPanel code={correctionCode} note={correctionNote} saved={correctionSaved} text={qualityText} styles={styles} onCode={setCorrectionCode} onNote={setCorrectionNote} onSave={() => void recordCorrection()} /><BudgetPanel spent={recommendation.solution.total} budget={recommendation.trace.intent.budget} locale={locale} text={budgetText} styles={styles} /><DecisionLedgerPanel trace={recommendation.trace} constraints={recommendation.solution.constraints} text={ledgerText} styles={styles} /></> : null}
          {completeRecommendation?.status === "abstain" ? <StatusCard title={completeText.abstain} body={completeRecommendation.reason} icon="verified-user" styles={styles} error /> : null}
          {completeRecommendation?.status === "solution" && activeCompleteStrategy ? <View style={styles.completeWrap}><View style={styles.strategyHeading}><Text style={styles.strategyLabel}>{completeText.strategy}</Text><Text style={styles.ownedNotice}>{completeText.solution}</Text></View><View style={styles.strategyRow}><Choice label={completeText.safe} active={selectedStrategy === "safe"} onPress={() => setSelectedStrategy("safe")} styles={styles} />{completeRecommendation.strategies.some((strategy) => strategy.id === "signature") ? <Choice label={completeText.signature} active={selectedStrategy === "signature"} onPress={() => setSelectedStrategy("signature")} styles={styles} /> : null}{completeRecommendation.strategies.some((strategy) => strategy.id === "statement") ? <Choice label={completeText.statement} active={selectedStrategy === "statement"} onPress={() => setSelectedStrategy("statement")} styles={styles} /> : null}</View><StrategyComparisonPanel strategies={completeRecommendation.strategies} locale={locale} text={comparisonText} styles={styles} /><Text style={styles.strategyDescription}>{activeCompleteStrategy.description}</Text><SolutionHeader recommendation={{ status: "solution", solution: activeCompleteStrategy.solution, trace: completeRecommendation.trace }} text={text} locale={locale} styles={styles} feedback={null} onFeedback={recordFeedback} showFeedback={false} onSave={(solution) => void saveSolution(ownedLabel || request, "complete", solution)} saveLabel={journalText.save} /><BudgetPanel spent={activeCompleteStrategy.solution.total} budget={completeRecommendation.trace.intent.budget} locale={locale} text={budgetText} styles={styles} /><DecisionLedgerPanel trace={completeRecommendation.trace} constraints={activeCompleteStrategy.solution.constraints} text={ledgerText} styles={styles} /></View> : null}
          {recreateAnalysis ? <RecreatePanel analysis={recreateAnalysis} text={recreateText} styles={styles} onUse={useInspirationForSearch} /> : null}
          {!recommendation && !loading && !error ? <View style={styles.empty}><MaterialIcons name="auto-awesome" size={22} color={colors.primary} /><Text style={styles.emptyText}>{text.noResult}</Text></View> : null}
        </>}
        renderItem={({ item }) => <PieceCard item={item} text={text} locale={locale} styles={styles} onPress={() => router.push({ pathname: "/product/[id]", params: { id: String(item.offer.id), name: item.offer.name, price: String(item.offer.price), currency: item.offer.currency, merchant: item.offer.merchantName, image: item.offer.imageUrl ?? "", link: item.offer.link, stock: item.offer.inStock === true ? "1" : item.offer.inStock === false ? "0" : "", category: item.offer.category ?? "" } } as never)} />}
        ItemSeparatorComponent={() => <View style={{ height: 12 }} />}
        ListFooterComponent={recommendation?.status === "solution" || completeRecommendation?.status === "solution" ? <View style={styles.footerSpace} /> : null}
      />
    </ScreenContainer>
  );
}
...
function createStyles(colors: ReturnType<typeof useColors>): Record<string, any> {
  const defined = StyleSheet.create({
    ...
    liveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#8FB072" },
