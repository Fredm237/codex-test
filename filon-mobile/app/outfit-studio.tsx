...
import { clearWardrobe, readWardrobe, removeWardrobeItem, saveWardrobeItem, type WardrobeItem } from "@/lib/filon-wardrobe";
...
const wardrobeCopy = {
  fr: { title: "Mon dressing", save: "Ajouter au dressing", empty: "Enregistrez vos pièces pour les réutiliser dans Complete.", use: "Utiliser", remove: "Retirer", clear: "Effacer tout mon dressing", local: "Conservé uniquement sur cet appareil." },
  nl: { title: "Mijn garderobe", save: "Aan garderobe toevoegen", empty: "Bewaar uw items om ze later in Complete te gebruiken.", use: "Gebruiken", remove: "Verwijderen", clear: "Mijn volledige garderobe wissen", local: "Alleen op dit apparaat bewaard." },
  en: { title: "My wardrobe", save: "Add to wardrobe", empty: "Save your pieces to reuse them in Complete.", use: "Use", remove: "Remove", clear: "Erase my entire wardrobe", local: "Kept only on this device." },
};
...
export default function OutfitStudioScreen() {
  ...
  const eraseWardrobe = async () => setWardrobe(await clearWardrobe());
...
function WardrobePanel({ items, text, styles, onSelect, onRemove, onClear }: { items: WardrobeItem[]; text: typeof wardrobeCopy.fr; styles: ReturnType<typeof createStyles>; onSelect: (item: WardrobeItem) => void; onRemove: (item: WardrobeItem) => Promise<void>; onClear: () => Promise<void> }) { return <View style={styles.wardrobePanel}><Text style={styles.wardrobeTitle}>{text.title}</Text><Text style={styles.wardrobeEmpty}>{text.local}</Text>{items.length === 0 ? <Text style={styles.wardrobeEmpty}>{text.empty}</Text> : <><WardrobeRow item={items[0]} text={text} styles={styles} onSelect={onSelect} onRemove={onRemove} /><WardrobeRow item={items[1]} text={text} styles={styles} onSelect={onSelect} onRemove={onRemove} /><WardrobeRow item={items[2]} text={text} styles={styles} onSelect={onSelect} onRemove={onRemove} /><Pressable accessibilityRole="button" accessibilityLabel={text.clear} onPress={() => void onClear()} style={({ pressed }) => [styles.wardrobeClear, pressed && styles.pressed]}><MaterialIcons name="delete-outline" size={15} color="#E59480" /><Text style={styles.wardrobeClearText}>{text.clear}</Text></Pressable></>}</View>; }
...
function createStyles(colors: ReturnType<typeof useColors>): Record<string, any> {
  const defined = StyleSheet.create({
    ...
    wardrobeClear: { minHeight: 44, marginTop: 12, paddingHorizontal: 10, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 7, borderRadius: 12, borderWidth: 1, borderColor: `${colors.error}55` },
    wardrobeClearText: { color: colors.error, fontSize: 11, fontWeight: "800" },
