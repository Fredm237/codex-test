...
const copy = {
  fr: { title: "Scanner un produit", intro: "Visez le code-barres pour rechercher les offres indexées dans le catalogue FILON.", back: "Retour", activate: "Autoriser la caméra", denied: "La caméra est désactivée. Vous pouvez autoriser l’accès ou saisir le code manuellement.", manual: "Saisir un code", placeholder: "EAN, UPC ou code produit", search: "Rechercher ce code", invalid: "Saisissez un code EAN, UPC ou produit valide.", unmatched: "Ce code ne correspond pas encore à un produit regroupé dans FILON.", unavailable: "Le catalogue est momentanément indisponible. Réessayez dans un instant.", frame: "Placez le code-barres dans le cadre", checking: "Recherche dans le catalogue FILON…", scan: "Lecture du code" },
  nl: { title: "Een product scannen", intro: "Richt op de barcode om aanbiedingen in de FILON-catalogus te zoeken.", back: "Terug", activate: "Camera toestaan", denied: "De camera is uitgeschakeld. Sta toegang toe of voer de code handmatig in.", manual: "Een code invoeren", placeholder: "EAN, UPC of productcode", search: "Deze code zoeken", invalid: "Voer een geldige EAN-, UPC- of productcode in.", unmatched: "Deze code komt nog niet overeen met een gegroepeerd product in FILON.", unavailable: "De catalogus is tijdelijk niet beschikbaar. Probeer het zo opnieuw.", frame: "Plaats de barcode in het kader", checking: "Zoeken in de FILON-catalogus…", scan: "Code lezen" },
  en: { title: "Scan a product", intro: "Point at the barcode to search offers indexed in the FILON catalogue.", back: "Back", activate: "Allow camera", denied: "The camera is disabled. Allow access or enter the code manually.", manual: "Enter a code", placeholder: "EAN, UPC or product code", search: "Search this code", invalid: "Enter a valid EAN, UPC, or product code.", unmatched: "This code does not yet match a grouped product in FILON.", unavailable: "The catalogue is temporarily unavailable. Please try again shortly.", frame: "Place the barcode inside the frame", checking: "Searching the FILON catalogue…", scan: "Code scan" },
};
  ...
  return (
    <ScreenContainer className="" containerClassName="bg-background" edges={["top", "left", "right", "bottom"]}>
      <View style={styles.content}>
        ...
        <DataReveal index={2}>
          {permission?.granted && Platform.OS !== "web" ? (
            ...
            <View style={styles.permission}>
              ...
              <Text style={styles.permissionKicker}>{text.scan}</Text>
        ...
        {status === "checking" ? <DataReveal index={4}><View style={styles.checking}><DataPulseTrack percentage={100} color={colors.primary} trackColor={colors.border} /><Text style={styles.verifying}>{text.checking}</Text></View></DataReveal> : null}
