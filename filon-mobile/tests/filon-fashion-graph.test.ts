...
const piece = (id: number, role: "base" | "structure" | "footwear" | "accessory") => ({ role, offer: offer(id), confidence: "not_calibrated" as const, provenance: "filon_inference" as const, explanation: { code: "piece.role_inferred" as const } });
...
describe("Fashion graph et critique", () => {
  it("établit des relations explicables sans altérer les offres", () => {
    ...
    expect(relations[0]).toMatchObject({ type: "COMPLEMENTS", fromOfferId: 1, toOfferId: 2, provenance: "filon_inference", score: null, confidence: "not_calibrated" });
  });
  ...
  it("signale une tenue contextuelle qui manque de structure au lieu de la présenter comme parfaite", () => {
    ...
    expect(critique.scorePenalty).toBeNull();
  });
});
