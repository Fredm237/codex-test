...
const piece = (id: number, role: "base" | "structure" | "footwear" | "accessory") => ({ role, offer: offer(id), confidence: "medium" as const, provenance: "filon_inference" as const, explanation: "Rôle interprété" });
...
describe("Fashion graph et critique", () => {
  it("établit des relations explicables sans altérer les offres", () => {
    ...
    const relations = buildFashionRelations(pieces, { request: "mariage", occasion: "Mariage", season: "Été", budget: 200, declaredStyle: null });
  ...
  it("signale une tenue contextuelle qui manque de structure au lieu de la présenter comme parfaite", () => {
    ...
    const relations = buildFashionRelations(pieces, { request: "mariage", occasion: "Mariage", season: "Été", budget: 200, declaredStyle: null });
    const critique = critiqueFashionComposition(pieces, { request: "mariage", occasion: "Mariage", season: "Été", budget: 200, declaredStyle: null }, relations);
