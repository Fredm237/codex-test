...
const outfit = (id: string, createdAt: string): SavedOutfit => ({ id, title: id, mode: "create", total: 80, currency: "EUR", confidenceScore: null, measurementStatus: "not_calibrated", pieces: [], createdAt });
...
describe("Rotation de tenues FILON", () => {
  it("propose d’abord les sauvegardes les plus anciennes sans déduire leur port réel", () => {
    ...
    expect(rotation[0].reason).toEqual({ code: "rotation.saved_days_ago", days: 46 });
    expect(resolveOutfitPublicMessage(rotation[0].reason, "fr")).toContain("reconsidérer");
  });
});
