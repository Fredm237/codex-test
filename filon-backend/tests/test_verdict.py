...
def _verdict(*, price, currency, history, **kwargs):
    timestamps = [captured_at for _, captured_at in history if captured_at is not None]
    return compute_verdict(
        ...
        in_stock=True,
        now=max(timestamps, default=BASE),
        **kwargs,
    )
...
class TestRefusesToGuess:
    def test_says_so_when_history_is_too_short(self):
        v = _verdict(price=100.0, currency="EUR", history=_history([100.0, 99.0]))
        assert v["level"] == "insuffisant"
        assert "trop récent" in v["headline"]
        assert v["confidence"] == "not_calibrated"

    def test_no_history_at_all(self):
        v = _verdict(price=100.0, currency="EUR", history=[])
        assert v["level"] == "insuffisant"
        assert v["samples"] == 0
    ...
    def test_many_samples_but_over_a_single_day_is_not_a_trend(self):
        ...
        v = _verdict(price=100.0, currency="EUR", history=history)
        assert v["level"] == "insuffisant"

    def test_never_claims_more_than_the_observed_window(self):
        v = _verdict(price=80.0, currency="EUR", history=_history([100.0] * 10 + [80.0]))
...
class TestVerdictLevels:
    def test_lowest_ever_is_excellent(self):
        ...
        assert "observations en stock" in v["reasons"][0]
        assert "aucun prix inférieur" in v["reasons"][0]
    ...
    def test_clearly_below_average_is_good(self):
        ...
        assert all("observations en stock" in reason for reason in v["reasons"])
...
class TestMerchantSpread:
    def test_cheaper_elsewhere_wins_with_one_fresh_observation(self):
        """L'écart entre marchands est mesurable dès le premier jour."""
        v = _verdict(
            price=150.0, currency="EUR", history=[(150.0, BASE)],
            cheapest_elsewhere=120.0, comparison_currency="EUR", merchants_count=3,
        )
        assert v["level"] == "attendre"
    ...
    def test_best_price_among_merchants_is_stated(self):
        v = _verdict(
            price=120.0, currency="EUR", history=[(120.0, BASE)],
            cheapest_elsewhere=120.0, comparison_currency="EUR", merchants_count=3,
        )
        assert "meilleur prix parmi les 3 marchands" in " ".join(v["reasons"])

    def test_single_merchant_says_nothing_about_competition(self):
        v = _verdict(price=120.0, currency="EUR", history=[], merchants_count=1)
        assert all("marchand" not in r for r in v["reasons"])


class TestConfidence:
    def test_confidence_remains_uncalibrated_regardless_of_coverage(self):
        short = _verdict(price=100.0, currency="EUR", history=_history([100.0] * 3))
        medium = _verdict(price=100.0, currency="EUR", history=_history([100.0] * 10))
        long = _verdict(price=100.0, currency="EUR", history=_history([100.0] * 40))
        assert short["confidence"] == "not_calibrated"
        assert medium["confidence"] == "not_calibrated"
        assert long["confidence"] == "not_calibrated"
