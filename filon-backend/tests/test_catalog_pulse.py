...
async def session():
    ...
    async with maker() as s:
        ...
        baisse = models.Offer(
            ...
            price=100.0, currency="EUR", image_url="https://e.test/i.jpg",
        )
        # Offre 2 : relevée à 80 €, vaut 80 € — stable.
        stable = models.Offer(
            merchant_id=m.id, awin_product_id="2", name="Stable",
            price=80.0, currency="EUR", image_url="https://e.test/i.jpg",
        )
        s.add_all([baisse, stable])
        await s.flush()

        s.add_all([
            models.PriceSnapshot(offer_id=baisse.id, price=120.0, captured_at=now - timedelta(hours=3)),
            models.PriceSnapshot(offer_id=baisse.id, price=100.0, captured_at=now - timedelta(minutes=20)),
            models.PriceSnapshot(offer_id=stable.id, price=80.0, captured_at=now - timedelta(hours=1)),
            # Hors fenêtre : ne doit compter nulle part.
            models.PriceSnapshot(offer_id=stable.id, price=300.0, captured_at=now - timedelta(days=4)),
        ])
        await s.commit()
        yield s
...
async def test_rend_la_date_du_dernier_releve(session):
    ...
    assert datetime.fromisoformat(res["last_reading"]) > datetime.utcnow() - timedelta(hours=1)
