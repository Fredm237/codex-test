...
export function LiquidMetal({ className, style }: { className?: string; style?: React.CSSProperties }) {
  ...
  useEffect(() => {
    ...
    const fs = [
      ...
      " vec3 teal=vec3(0.05,0.72,0.68),silver=vec3(0.95,0.97,1.0),blue=vec3(0.10,0.44,0.90);",
      " vec3 c=mix(teal,silver,smoothstep(0.0,0.5,y));c=mix(c,blue,smoothstep(0.55,1.0,y));",
      " c+=vec3(1.0)*pow(max(dot(r,normalize(vec3(0.4,0.85,0.5))),0.),60.);return c;}",
