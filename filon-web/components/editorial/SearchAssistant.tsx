...
const SL = {
  fr: {
    ...
    marketNodes: [["MARCHANDS", "sources séparées"], ["IDENTITÉ", "preuve requise"], ["PRIX", "devise explicite"]],
  ...
  nl: {
    ...
    marketNodes: [["WINKELS", "aparte bronnen"], ["IDENTITEIT", "bewijs vereist"], ["PRIJS", "expliciete valuta"]],
  ...
  en: {
    ...
    marketNodes: [["MERCHANTS", "separate sources"], ["IDENTITY", "evidence required"], ["PRICE", "explicit currency"]],
  ...
  return (
    <section className={`sa ${marketStyles.surface} ${phase !== "idle" ? "searched" : ""}`} data-market-state={phase}>
      <div className={marketStyles.field} aria-hidden="true">
        {S.marketNodes.map(([label, detail]) => (
          <span className={marketStyles.node} key={label}>{label}<small>{detail}</small></span>
        ))}
        <span className={marketStyles.ring} />
        <span className={marketStyles.axis} />
      </div>
      <div className={`ed-wrap sa-content ${marketStyles.content}`}>
        <span className={marketStyles.plan} aria-hidden="true">FILON / MARCHÉ 01</span>
