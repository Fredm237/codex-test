...
| 28–29 | Observabilité et opérations | **PARTIEL** | Corrélation des huit jalons, spans dépendances, propagation W3C, exporteur OTLP/HTTP borné, probes, métriques et identité `X-Real-IP` qualifiés ; Redis privé production répond `ok` sans fallback ; Cron privé EU West a passé le préflight, porte une cadence 6 h et exécute un premier cycle Awin réel journalisé ; collecteur/rétention de traces, agrégateur/dashboard, pager, trafic représentatif et état terminal du cycle courant manquent |
