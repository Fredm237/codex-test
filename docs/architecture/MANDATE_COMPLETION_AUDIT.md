...
| 28–29 | Observabilité et opérations | **PARTIEL** | Corrélation des huit jalons décisionnels, spans PostgreSQL/Redis/Awin/SerpAPI/LLM, propagation W3C HTTP, exporteur OTLP/HTTP opt-in borné et sans payload, probes, métriques, identité `X-Real-IP` Railway réelle et anti-spoofing qualifiés, Redis atomique opt-in, scheduler mono-exécution et préflight non-écrivant, alertes locales, OpenMetrics, pack Prometheus/Grafana et activation fail-closed validés ; service Cron, collecteur/rétention de traces, activation Redis production ou WAF, déploiement agrégateur/dashboard, reçu, pager et trafic représentatif manquent |
