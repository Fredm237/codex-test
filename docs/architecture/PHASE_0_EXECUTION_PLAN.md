...
| P0.7 — CI complète — **TERMINÉ** | Matrice de qualité multi-surfaces | Workflow publié ; readiness téléversée ; branche byte-identique ; run #362 exécuté ; preview Vercel construite ; ruleset `21798272` active sur `main`, PR et quatre jobs requis sans bypass | Alembic, 2 124 régressions backend, readiness, web/mobile/extension verts ; artefact `9738761749` inspectable ; gate strict rouge uniquement sur le NO-GO humain attendu | L'intégrité invalide et le `not_ready` sur changement moteur restent bloquants ; aucun bypass administrateur |
