...
**P0.6 Observabilité est partiellement qualifiée en production** : corrélation HTTP, huit jalons décisionnels, spans PostgreSQL/Redis/Awin/SerpAPI/LLM, propagation W3C, téléchargement Awin borné, percentiles, readiness fail-closed, moteur local de cinq règles, front door stricte, limite Redis atomique et export OpenMetrics 1.0 sont testés. L'exporteur OTLP/HTTP officiel reste désactivé faute de collecteur. Redis privé est désormais activé sans fallback et sa sonde réelle répond `ok`. Le service privé `filon-catalog-cron` a passé son préflight expurgé, utilise la cadence `0 */6 * * *` UTC et son premier cycle réel écrit progressivement dans PostgreSQL tout en laissant le web `ready`. Le [reçu Redis/Cron](PHASE_06_REDIS_CRON_ACTIVATION_REPORT.md) conserve les identifiants, résultats et limites. Le pack Prometheus/Grafana demeure non déployé. Collecteur/backend de traces, agrégateur, scrapes/rétention, dashboard, pager et trafic représentatif manquent encore. **P0.7 CI est terminé** : branche publique byte-identique, Actions #366, artefact Quality `9739367304`, preview Vercel et ruleset `21798272` active sur `main` sans bypass sont prouvés dans le [rapport distant](PHASE_0_REMOTE_QUALIFICATION_REPORT.md).
...
| P0.6 — Observabilité — **EN COURS** | Corrélation, readiness, front door, exports OpenMetrics/OTLP, scheduler, traces de bout en bout et dashboards | Request ID, latences P50/P95/P99, huit jalons décisionnels, spans et `traceparent`, compteurs bornés, front door stricte, Redis atomique sans fallback, OpenMetrics, OTLP borné, pack Prometheus/Grafana et scheduler mono-exécution livrés | Redis privé réel `ok` ; Cron privé EU West, préflight `ready`, cadence 6 h, journal persistant et premier cycle Awin réel en cours ; OTLP/config/transport 89/89 et backend logique 2 132 + 2 ignorés ; sortie après état terminal du cycle, collecteur, scrapes/rétention, dashboard et notifications | Redis et Cron sont actifs ; OTLP reste désactivé sans collecteur ; agrégateur, réception externe de trace, pager et trafic représentatif manquent ; rollback Redis/Cron sans migration documenté |
