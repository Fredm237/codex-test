...
| P0.7 — CI complète — **TERMINÉ** | Matrice de qualité multi-surfaces | Workflow publié ; readiness téléversée ; branche byte-identique ; run #354 exécuté ; preview Vercel construite ; ruleset `21798272` active sur `main`, PR et quatre jobs requis sans bypass | Alembic, backend, readiness, web/mobile/extension verts ; artefact `9737603091` inspectable ; gate strict rouge uniquement sur le NO-GO humain attendu | L'intégrité invalide et le `not_ready` sur changement moteur restent bloquants ; aucun bypass administrateur |
