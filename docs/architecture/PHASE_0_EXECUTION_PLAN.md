...
La requalification courante du funnel est Actions **#356** (`33332272585`) :
web, mobile, extension, Alembic, les **2 118** régressions backend et la
readiness normale sont verts. Le gate strict reste rouge uniquement parce que
les sept datasets humains sont vides. L'artefact Quality `9738018876`, digest
...
| P0.7 — CI complète — **TERMINÉ** | Matrice de qualité multi-surfaces | Workflow publié ; readiness téléversée ; branche byte-identique ; run #356 exécuté ; preview Vercel construite ; ruleset `21798272` active sur `main`, PR et quatre jobs requis sans bypass | Alembic, backend, readiness, web/mobile/extension verts ; artefact `9738018876` inspectable ; gate strict rouge uniquement sur le NO-GO humain attendu | L'intégrité invalide et le `not_ready` sur changement moteur restent bloquants ; aucun bypass administrateur |
