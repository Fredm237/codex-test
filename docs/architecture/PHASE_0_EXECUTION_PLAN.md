...
**P0.2 Quality Lab reste en cours** : le contrat v0.5 ferme le roster à sept datasets et 27 gates, exécute les moteurs sur une entrée aveugle, lie l'identité du run au manifeste/adaptateurs/sorties, empreinte le contenu exact du holdout et compare deux scorecards sans accepter de roster tronqué. La publication du run est atomique et sans remplacement. Les sept adaptateurs applicatifs sont maintenant branchés : les nouveaux adaptateurs Graph utilisent le resolver conservateur exact-GTIN, sans similarité de titre, marque ou catégorie. Les **377 tests Quality** et la suite backend courante (**2 091 réussis + 2 ignorés**) sont verts. La readiness normale sort avec 0 et la stricte avec 1, mais l'état réel reste `integrity_valid=true`, `ready=false`, `status=not_ready` avec **0 cas humain**. Aucun rapport de régression métier n'est donc mesurable et aucune confiance n'est calibrée.

**P0.5 Product/Variant, Offer Graph et Merchant Intelligence sont livrés en shadows techniques locaux** : dix tables d'expansion, trois migrations expand-only, des writers isolés et des backfills bornés/idempotents sont disponibles derrière quatre flags désactivés par défaut. L'identité utilise uniquement `exact-gtin-shadow-v1`. L'Offer Graph conserve argent décimal, devise, stock tri-state et lien HTTPS public. Merchant Intelligence expose des compteurs à dénominateur explicite et laisse livraison, retours, garantie, support, paiement, shipping, exactitude prix et mismatch stock `not_measurable` ; aucun score n'est fabriqué. Les lectures v1 restent inchangées. Ce lot ne constitue ni un backfill de production ni un GO métier : les seuils restent non mesurables sans holdout humain.
...
| P0.5 — Product/Variant + Offer Graph + Merchant Intelligence shadows — **EN COURS** | Brand/Family/Model/Variant/Offer/Merchant v0 | Dix tables expand-only, backfills contrôlés, résolution exacte-GTIN, preuves offre décimales et compteurs marchands sans score livrés localement | Technique : migrations/replay/adaptateurs verts ; métier : seuils entity/variant/attachment/offer truth et qualité marchand atteints sur tests indépendants | Flags off par défaut ; aucun consommateur v2 ; pas de backfill production ; downgrade seulement sur environnement éphémère |
