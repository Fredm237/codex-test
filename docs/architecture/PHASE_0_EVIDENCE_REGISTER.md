...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; backend Core, probes, export direct et capacité volume production verts, agrégation externe encore NO-GO | Déploiement Railway `c832d1c8…` : live/ready HTTP 200, OpenMetrics 1.0 qualifié, volume 39,28 %, backups et moniteurs 14/17 GB actifs ; Redis atomique opt-in + identité `X-Real-IP` sans adresse brute ni fallback : **138 ciblés**, backend **2 065 réussis + 2 ignorés**, non activé en production | Readiness publique, contrat OpenMetrics, audit Railway ; concurrence Redis, panne/plafond, en-tête absent/dupliqué/invalide, séparation derrière un pair, confidentialité, configuration et cycle de vie | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [politique front door](FRONT_DOOR_SECURITY_POLICY.md), [reçu OpenMetrics/résilience](PHASE_06_PRODUCTION_OBSERVABILITY_RECEIPT.md), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md), [pack agrégateur](../../filon-backend/observability/README.md) | Déployer backend de traces et agrégateur ; vérifier `X-Real-IP` puis activer/tester Redis privé ou WAF ; canal/pager, notification réelle et trafic représentatif restent ouverts |
