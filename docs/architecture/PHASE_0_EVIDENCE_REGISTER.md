...
| P0.5 — Product/Variant Graph shadow | `p0_f` | **EN COURS — SHADOW TECHNIQUE LIVRÉ, GATE HUMAIN ROUGE** | Huit tables `graph_*`, migration `8b2f4c7d9a10`, resolver `exact-gtin-shadow-v1`, writer Awin isolé, backfill dry-run/apply borné et sept adaptateurs Quality ; backend **2 078 + 2 ignorés** | 125 tests Graph/migration/configuration ciblés + 2 PostgreSQL opt-in ignorés ; Quality 377/377 ; readiness stricte exit 1 attendu | [ADR Product Graph](ADR-002-PRODUCT-GRAPH-IDENTITY-SHADOW.md), [rapport P0.5](PHASE_0F_PRODUCT_GRAPH_SHADOW_REPORT.md), [runbook](DATABASE_MIGRATION_RUNBOOK.md) | Aucun backfill ni activation production ; dépend de P0.2 pour mesurer faux merges/splits, relation de variante et attachement sur un holdout indépendant |
