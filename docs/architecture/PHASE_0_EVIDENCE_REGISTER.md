...
- Référence locale versionnée : `56de1cf`
- Référence Quality versionnée : `6e12386`
...
|---|---|---|---|---|---|---|
| P0.0 — Freeze, ownership et baseline | `p0_a` | **AUDIT TERMINÉ** (`p0_a` fait) ; gouvernance distante ouverte | Audit local et GitHub consigné ; backend **1 186/1 186**, web **17/17** et build, extension syntaxe/manifeste verts ; dette mobile explicitement conservée | `python -m pytest -q` dans le backend, puis tests/builds de chaque surface | [Current System Map](CURRENT_SYSTEM_MAP.md), [baseline qualité](BASELINE_QUALITY_REPORT.md), [matrice de décision](KEEP_REFACTOR_REWRITE_DELETE.md) | La preuve de baseline est acquise ; le gel reste applicable jusqu'au GO ; protection `main` et checks requis restent à établir à distance |
| P0.1 — Contrats v1 et `unknown` | `p0_b` | **EN COURS** ; consommateurs non protégés versionnés, intégration protégée prévalidée mais non autorisée | `e152ed0`, `4e5755d` et `56de1cf` ferment les montants publics sans preuve ; copie isolée du lot protégé : backend **1 928 réussis / 1 ignoré**, web **17/17**, typecheck et build 42 pages verts | Suites de compatibilité backend/web/mobile/extension ; gates vérité produit ; preuve explicite, `observed_at` valide ≤ 72 h, devise compatible, stock, prix, URL sûre et revalidation Core exigés avant action | [prévalidation protégée](PROTECTED_TRUTH_INTEGRATION_PREFLIGHT.md), [registre `contracts/v1`](../../contracts/v1/README.md), [claims non supportés](UNSUPPORTED_CLAIMS_REGISTER.md), [preuve d'invariance](AFFILIATE_INVARIANCE_REPORT.md) | Autorisation ciblée puis intégration de `catalog.py` et `SearchAssistant.tsx` ; tant qu'ils manquent, les clients s'abstiennent ; le TTL 72 h reste provisoire |
| P0.2 — Quality Lab | `p0_c` | **EN COURS** ; infrastructure v0.5 locale verte, données **NO-GO** | Le roster reste fermé à 7 datasets/27 gates ; le runner aveugle branche désormais 5 moteurs réels, dont Decision déterministe et sourcé ; **359/359 tests Quality Lab** et backend courant **2 012 réussis + 1 ignoré** ; rapport `integrity_valid: true`, `ready: false` | Suites Quality complètes ; readiness normale exit 0 et stricte exit 1 ; runner, scorecard puis comparateur seulement sur artefacts complets | [manifeste v0.5](../../quality/manifest.json), [rapport de readiness daté](../../quality/reports/readiness-2026-08-29.json), [rapport P0.c](PHASE_0C_QUALITY_REPORT.md) | Collecter les sept jeux réellement indépendants avec adjudication, provenance, volumes et supports minimaux ; brancher les 2 adaptateurs Graph encore absents ; aujourd'hui : **0 cas humains** et aucune régression métier mesurable |
...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; fondation, traces applicatives, export et pack agrégateur local verts, production **NO-GO** | Corrélation **8/8 tests**, sûreté Awin **11 nouveaux tests** et backend courant **2 012 + 1 ignoré** ; huit jalons, spans PostgreSQL/Redis/Awin/SerpAPI/LLM, `traceparent` HTTP et download Awin borné ; activation Prometheus couverte par 49 tests, 11 règles lintées et scénario multi-réplica vert | Suites confidentialité/concurrence/erreurs/propagation/limites, middleware/probes/alertes/front door/export/activation ; champs fermés, corps compressé/décompressé/lignes plafonnés, scrape fail-closed et reçu sans hôte/secret | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [pack agrégateur](../../filon-backend/observability/README.md), [contrat métriques v2](OBSERVABILITY_METRICS_CONTRACT.md), [politique front door](FRONT_DOOR_SECURITY_POLICY.md) | Déployer backend de traces et agrégateur approuvés, produire le reçu et vérifier rétention/resets ; CIDR Railway, WAF/limite distribuée, canal/pager et trafic représentatif restent externes |
| P0.7 — CI multi-surfaces et protection | `p0_g` | **EN COURS** ; simulation intégrée verte, preuve distante absente | Copie isolée : backend **1 928 réussis + 1 ignoré**, web **17/17**, typecheck et build 42 pages ; mobile versionné **326 réussis + 4 ignorés**, typecheck et lint 0/17 ; extension verte | Workflow local backend/contrats/migrations/Quality, web, mobile et extension ; sortie Quality invalide = code `2` | [prévalidation protégée](PROTECTED_TRUTH_INTEGRATION_PREFLIGHT.md), [workflow Phase 0](../../.github/workflows/backend-catalog-quality.yml), [baseline CI locale](BASELINE_QUALITY_REPORT.md) | Workflow non publié, aucun run distant observé, lots protégés non autorisés/non intégrés, protection/checks requis de `main` absents |
...
- `e152ed0`, `4e5755d` et `56de1cf` ajoutent les primitives de vérité prix et
  masquent les montants des cartes, rails et fiches web sans preuve courante.
  La [prévalidation protégée](PROTECTED_TRUTH_INTEGRATION_PREFLIGHT.md) rejoue
  les changements `catalog.py`, SearchAssistant et MegaMenu dans une copie
  isolée : **1 928 tests backend réussis + 1 ignoré**, web **17/17**, typecheck
  et build de 42 pages verts. Ce résultat ne constitue ni un commit des
  fichiers protégés, ni une preuve CI distante, ni une mesure humaine.
...
l'infrastructure de production n'est pas qualifiée et que `catalog.py` et
`SearchAssistant.tsx` ne sont pas intégrés. Le contrat UTC naïf du catalogue
et la concordance d'URL canonique Assistant/catalogue restent ouverts.
