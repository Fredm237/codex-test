...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; backend Core, probes, identité edge, exports opt-in, capacité volume et préflight scheduler verts ; infrastructures externes encore NO-GO | Identité edge et OpenMetrics 1.0 qualifiés ; exporteur OTLP/HTTP officiel, échantillonné et sans auto-instrumentation : **88/88 ciblés**, backend **2 131 réussis + 2 ignorés** ; image Railway `b591c7cd…` prête et OTLP désactivé, Redis/Cron non activés | Spans mémoire concordants avec W3C, aucun argument/retour/message d'exception ; configuration partielle refusée, lots et timeout bornés, fermeture testée ; readiness publique, contrat OpenMetrics, audit Railway, préflight scheduler et résilience Redis | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [export OTLP](PHASE_06_OTLP_TRACE_EXPORT_REPORT.md), [préflight scheduler](PHASE_06_SCHEDULER_PREFLIGHT_REPORT.md), [politique front door](FRONT_DOOR_SECURITY_POLICY.md), [reçu OpenMetrics/résilience](PHASE_06_PRODUCTION_OBSERVABILITY_RECEIPT.md), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md), [pack agrégateur](../../filon-backend/observability/README.md) | Approuver/déployer un collecteur et qualifier réception/rétention/suppression ; déployer le Cron et l'agrégateur ; activer/tester Redis privé ou WAF ; canal/pager et trafic représentatif restent ouverts |
| P0.7 — CI multi-surfaces et protection | `p0_g` | **TERMINÉ** (`p0_g` fait) | Référence applicative distante `216f38b`, arbre `11e0956…` identique au commit local `0bd6bad` ; PR #385 ; Actions #364 : web/mobile/extension verts, Alembic, 2 131 régressions backend et readiness normale verts ; seul le gate strict Quality échoue comme attendu sur les datasets humains vides ; artefact `9739162091` | Workflow backend/contrats/migrations/Quality, web, mobile et extension ; GitHub Actions réel et protection de `main` | [qualification distante](PHASE_0_REMOTE_QUALIFICATION_REPORT.md), [workflow Phase 0](../../.github/workflows/backend-catalog-quality.yml), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md) | Lot acquis ; la conclusion globale reste NO-GO parce que le check backend ferme correctement sur les datasets humains vides |
