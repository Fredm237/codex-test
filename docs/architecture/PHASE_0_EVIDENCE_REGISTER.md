...
- Référence applicative locale qualifiée : `34a4f85`
- Référence applicative distante qualifiée : `7a39348`, arbre commun `1706a0be2418173a6fb68782c01ae38b2e1f12d2`
- Référence Quality versionnée : artefact Actions `9738018876`
...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; backend Core, probes, identité edge, export direct, capacité volume et préflight scheduler verts ; infrastructures externes encore NO-GO | Déploiement Railway `03be13dc…` actif en EU West : live/ready et `/health` protégé HTTP 200, valeurs `X-Real-IP` forgées écrasées ; OpenMetrics 1.0 qualifié, volume 39,28 %, backups et moniteurs 14/17 GB actifs ; préflight scheduler non-écrivant **14/14**, backend **2 124 réussis + 2 ignorés** ; Redis et service Cron non activés | Readiness publique, route protégée et probes anti-spoofing réelles ; contrat OpenMetrics, audit Railway ; scheduler refuse secrets absents, schéma legacy et plafonds dangereux sans lancer l'ingestion ; concurrence Redis, panne/plafond, en-tête absent/dupliqué/invalide, séparation derrière un pair, confidentialité, configuration et cycle de vie | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [préflight scheduler](PHASE_06_SCHEDULER_PREFLIGHT_REPORT.md), [politique front door](FRONT_DOOR_SECURITY_POLICY.md), [reçu OpenMetrics/résilience](PHASE_06_PRODUCTION_OBSERVABILITY_RECEIPT.md), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md), [pack agrégateur](../../filon-backend/observability/README.md) | Déployer et qualifier le service Cron ; déployer backend de traces et agrégateur ; activer/tester Redis privé ou WAF ; canal/pager, notification réelle et trafic représentatif restent ouverts |
| P0.7 — CI multi-surfaces et protection | `p0_g` | **TERMINÉ** (`p0_g` fait) | Référence distante `7a39348`, arbre `1706a0…` identique au commit local `34a4f85` ; PR #385 ; Actions #356 : web/mobile/extension verts, Alembic, 2 118 régressions backend et readiness normale verts ; seul le gate strict Quality échoue comme attendu sur les datasets humains vides ; artefact `9738018876` | Workflow backend/contrats/migrations/Quality, web, mobile et extension ; GitHub Actions réel et protection de `main` | [qualification distante](PHASE_0_REMOTE_QUALIFICATION_REPORT.md), [workflow Phase 0](../../.github/workflows/backend-catalog-quality.yml), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md) | Lot acquis ; la conclusion globale reste NO-GO parce que le check backend ferme correctement sur les datasets humains vides |
