...
| P0.2 — Quality Lab | `p0_c` | **EN COURS** ; infrastructure v0.5 locale et distante verte, données **NO-GO** | Le roster reste fermé à 7 datasets/27 gates ; 5 moteurs réels sont branchés, dont Decision déterministe et sourcé ; **359/359 tests Quality** locaux, backend final **2 020 + 1 ignoré**, Actions #343 **2 021/2 021** ; rapport `integrity_valid: true`, `ready: false` | Suites Quality complètes ; readiness normale exit 0 et stricte exit 1 ; artefact distant inspectable | [manifeste v0.5](../../quality/manifest.json), [rapport de readiness daté](../../quality/reports/readiness-2026-08-29.json), [rapport P0.c](PHASE_0C_QUALITY_REPORT.md) | Collecter les sept jeux réellement indépendants avec adjudication, provenance, volumes et supports minimaux ; brancher les 2 adaptateurs Graph encore absents ; aujourd'hui : **0 cas humains** et aucune régression métier mesurable |
...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; fondation, traces applicatives, export et pack agrégateur local verts, production **NO-GO** | `dfe9098` borne aussi mémoire, volume compressé/décompressé et lignes Awin ; local **2 020 + 1 ignoré**, Actions #343 **2 021/2 021**. Le health Railway observé reste `env=dev`, Redis local et Qdrant désactivé | Suites confidentialité/concurrence/erreurs/propagation/limites, middleware/probes/alertes/front door/export/activation ; lecture réelle de `/health` | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [pack agrégateur](../../filon-backend/observability/README.md), [qualification distante](PHASE_0_REMOTE_QUALIFICATION_REPORT.md) | Déployer backend de traces et agrégateur approuvés, produire le reçu et vérifier rétention/resets ; CIDR Railway, WAF/limite distribuée, canal/pager et trafic représentatif restent externes |
