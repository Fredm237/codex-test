...
| P0.6 — Observabilité et front door | `p0_i` | **EN COURS** ; backend Core, probes, export direct et capacité volume production verts, agrégation externe encore NO-GO | Déploiement Railway `c832d1c8…` : Dockerfile, preDeploy Alembic, `env=production`, live/ready HTTP 200 ; export : **401/401/401/200**, OpenMetrics 1.0, `no-store`, 24 familles et 15 labels fermés ; volume 39,28 %, plannings `DAILY + WEEKLY + MONTHLY` actifs ; moniteurs `DISK_USAGE_GB above 14/17 GB`, sans alerte active | Readiness publique, contrat OpenMetrics authentifié, audit API des sauvegardes, dashboards, cibles et seuils | [rapport P0.6](PHASE_06_OBSERVABILITY_REPORT.md), [reçu OpenMetrics/résilience](PHASE_06_PRODUCTION_OBSERVABILITY_RECEIPT.md), [reçu Railway](PHASE_0_RAILWAY_DEPLOYMENT_RECEIPT.md), [pack agrégateur](../../filon-backend/observability/README.md) | Déployer backend de traces et agrégateur, produire le reçu de scrapes, vérifier rétention/resets ; WAF/limite distribuée, canal/pager, livraison réelle des notifications et trafic représentatif restent ouverts |
