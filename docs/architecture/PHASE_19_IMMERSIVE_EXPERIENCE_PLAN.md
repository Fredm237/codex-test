...
- État : **P19A–E et propagation candidate qualifiés en Preview ; direction validée par le fondateur ; aucune publication production**
...
| 12. Revues visuelle/performance/a11y | **PASS PREVIEW** | build, desktop, mobile, clavier, fallback, budget et garde runtime verts ; mesure terrain publique requise avant production |
| 13. Choix du langage final | **PASS** | B — chorégraphie de preuve |
| 14. Expérience complète | **GO PREVIEW / NO-GO PRODUCTION** | noyau inter-routes, home, laboratoire et dossier exact publiés sur une Preview Vercel temporaire non indexée |
...
La revue mobile de la couche WebGL est désormais confirmée en Preview à
**390 × 844 px** : un canvas différé, produit exact lisible, aucun débordement
horizontal et aucune erreur navigateur. Elle ne transforme aucun état inconnu
en preuve positive.
...
Le simple sparkline décoratif est ensuite devenu un paysage prix-temps piloté
par les relevés comparables réels. Un curseur natif permet de parcourir chaque
observation ; la valeur et la date actives sont annoncées, et un tableau HTML
dépliable conserve l'intégralité de la lecture accessible. Les points sans
devise explicite, sans stock positif ou avec une date invalide restent exclus
par `comparablePriceHistory` avant tout rendu.

La fiche réelle `70505` a été rejouée localement avec ses deux observations
EUR conservées, son verdict `insuffisant`, son image NANKANG et son lien vers le
dossier EAN. Le transfert `offre → produit exact` transporte cette même image,
sans interrompre la navigation. La suite web, TypeScript et le build de 47
routes sont verts ; la route offre mesure désormais **7,38 kB / 165 kB First
Load JS**. Aucun canvas, verdict, prix ou point historique synthétique n'a été
ajouté. La revue mobile du paysage est désormais intégrée à la Preview : un
curseur, deux lignes de relevés, aucune largeur excédentaire et aucune erreur
navigateur.
...
4. **PASS Preview pour le dossier EAN ; PASS code/build pour l'offre historique** — raccorder le dossier EAN et
   l'offre historique au chapitre `identity → proof`, en conservant EAN,
   marchands, dates, périmètre exact et unknown dans le DOM ;
5. **PASS code/build ; Score mobile revu** — raccorder score/cashback aux
...
8. **PASS Preview pour tests, TypeScript, build, budgets, invariants de vérité,
   clavier et vues rejouées sans débordement** — rejouer clavier, desktop et mobile dans le
...
### Reçu de qualification Preview et continuité d'objet — 3 septembre 2026

La continuité inter-routes transporte désormais l'image réelle sélectionnée,
sa géométrie et son libellé depuis la home, le laboratoire ou une carte catalogue
jusqu'au dossier produit exact. Il ne s'agit plus d'un simple balayage générique :
le même objet rejoint la cible DOM du plan `identity`. Le lien reste une navigation
Next.js normale et le transfert disparaît totalement lorsque le mouvement réduit
est demandé, lorsque l'image manque ou lorsque JavaScript n'est pas disponible.

Preuves du candidat publié :

- commit GitHub **`4e47b30d25aa210fac837e3ef6a9ebe966dd4842`** sur
  `codex/filon-immersive-production`, sans fusion dans `main` ;
- déploiement Vercel **`2gD9QAoctGDA1zpWvKKZ8JgKPoJA`**, état terminal
  **Ready**, environnement **Preview** ;
- URL immuable :
  `https://codex-test-jqj97n24b-teamfilon.vercel.app/laboratoire/experience/` ;
- protection Vercel active et en-tête **`x-robots-tag: noindex`** ;
- suite web complète, TypeScript et build Next.js de **47 routes** verts ;
- laboratoire : produit NANKANG exact, EAN `4717622052664`, six offres et cinq
  marchands, quatre plans pilotables, slider et lien d'évitement présents dans
  le DOM ;
- home desktop : un canvas différé, aucune largeur excédentaire et aucune erreur
  navigateur ; transfert observé avec la texture réelle
  `957496_5d8a4c90_medium.jpg`, puis réception sur la cible du dossier EAN ;
- matrice mobile du runtime immédiatement antérieur, sans changement de scène :
  **390 × 844**, aucun débordement, WebGL `full`, LCP **872 ms**, CLS **0**,
  tâche longue immersive **0 ms** ;
- mesure desktop chaude du même runtime : LCP **1 152 ms**, CLS **0,0231**,
  interaction **200 ms**, tâche longue immersive **0 ms**. Le premier chargement
  froid derrière la protection SSO a atteint **3 324 ms** : il reste exclu d'une
  preuve de production publique et devra être requalifié sur un canary sans cette
  couche externe.

Décision : **GO Preview** pour poursuivre la réalisation complète. **NO-GO
production** tant que la matrice finale des routes publiques, la mesure terrain
du canary et l'autorisation explicite de promotion ne sont pas réunies.

### Reçu Preview — recherche vers preuve exacte — 3 septembre 2026

Le plan `market` ne sort plus directement vers le marchand lorsqu'un résultat
porte un EAN ou un identifiant d'offre réel. Chaque carte propose d'abord
`Ouvrir la preuve` et transporte l'image et le libellé vers le dossier exact ;
le lien marchand demeure distinct, externe et affilié. Les cartes sans prix,
devise, stock ou fraîcheur admissibles restent absentes.

La Preview statique relaie désormais le flux SSE sur sa propre origine. Le
backend reste la seule source des événements et aucune réponse de secours n'est
créée. La route serveur Next et la réécriture Vercel couvrent respectivement le
mode applicatif et l'export statique.

Preuves du candidat publié :

- commit GitHub **`b9a1807c71e6ec0430a21f472b96736f5d71a3ca`**, sans
  fusion dans `main` ;
- déploiement Vercel **`CG5wAnTXDjHTJrf9Yu9D7jNJLz6y`**, état terminal
  **Ready**, environnement **Preview** ;
- URL immuable :
  `https://codex-test-q8raaaf65-teamfilon.vercel.app/` ;
- protection SSO active et en-tête **`x-robots-tag: noindex`** ;
- recherche réelle `NANKANG Winter Activa SV-3` : **318 offres analysées**,
  **5 cartes** avec preuve courante et **5 passages** internes vers un EAN ;
- navigation vérifiée vers l'EAN exact `4717622052466`, puis dossier produit
  affichant **6 offres comparables** et **5 marchands** ;
- mobile **390 × 844** : cinq cartes et cinq liens de preuve, aucun débordement
  horizontal, aucun canvas requis et aucune erreur ou alerte navigateur ;
- suite web complète, TypeScript et build Next.js de **47 pages** verts ; le
  flux local a transmis un événement terminal `results` réel sans altération.

Décision : **GO Preview** pour la continuité `market → identity → proof`.
La promotion production reste hors périmètre et non autorisée.

### Reçu Preview — outils mobiles non recouverts — 3 septembre 2026

L'audit visuel de l'offre `70505` à **390 × 844 px** a détecté que le CTA mobile
fixe recouvrait la valeur sélectionnée et le curseur du paysage prix-temps. Le
contrôle global masque désormais ce CTA lorsqu'une zone interactive protégée,
un champ actif ou le footer est visible. Il est aussi absent de Recherche, où
sa destination serait redondante, puis réapparaît hors de ces zones.

Preuves du candidat publié :

- commit GitHub **`19658bfe3846fe336cd9345d6c6ea5fe7658ddf5`**, sans
  fusion dans `main` ;
- déploiement Vercel **`9QrL2stbVUpUbqCnuWseVtvHg3kw`**, état terminal
  **Ready**, environnement **Preview** ;
- URL immuable : `https://codex-test-ik4bmrd5m-teamfilon.vercel.app/` ;
- home mobile : produit réel et canvas différé visibles, recherche disponible,
  largeur document **384 px** dans un viewport **390 px** ;
- dossier EAN mobile : produit réel, un canvas, six offres comparables et cinq
  marchands, sans débordement ni erreur navigateur ;
- offre historique mobile : paysage réel, curseur et deux relevés accessibles,
  CTA fixe masqué pendant l'interaction puis restauré hors de la zone ;
- Outfit Studio : table de saisie protégée ; footer : newsletter et registre de
  sortie protégés ; Recherche : aucun CTA redondant rendu ;
- suite web complète, TypeScript, `git diff --check` et build Next.js de
  **47 pages** verts.

Décision : **GO Preview mobile** pour les surfaces auditées. Aucun effet sur
`main`, aucun déploiement production et aucune activation de reader shadow.

### Qualification mobile transversale et terminaison Recherche — 3 septembre 2026

La matrice mobile de la Preview a été étendue à Marchands, Score, Cashback,
Codes promo, Reconditionné, Intelligence, Comment ça marche, Transparence,
Sécurité, Blog, Contact et Outfit Studio. À **390 × 844 px**, chacune conserve
une largeur document de **384 px**, un titre principal visible et aucun
débordement horizontal. Cette passe a aussi révélé deux écarts vérifiables :

- Marchands et Rayon imbriquaient un second élément `main` dans celui du layout ;
  les deux surfaces gardent désormais une unique zone principale ;
- le client Recherche coupait son SSE à 30 secondes alors que le backend peut
  émettre ses huit étapes puis consacrer jusqu'à 30 secondes au calcul. Une
  mesure directe a observé l'événement terminal à **32,66 s**. La limite client
  est donc portée à **40 s** afin de recevoir le résultat ou l'abstention métier,
  sans étendre le budget serveur ni fabriquer une réponse.

Le rendu de production local confirme une seule zone `main` sur Marchands et
Rayon. Sur la même requête réelle NANKANG, l'abstention terminale
« Aucune offre vérifiée » est désormais reçue ; elle ne se transforme plus en
fausse panne de transport. Suite web complète, TypeScript, `git diff --check`
et build Next.js de **47 pages** sont verts. Cette qualification reste locale :
elle n'autorise ni fusion, ni production, ni activation de reader shadow.

### Gate accessibilité et performance du candidat local — 3 septembre 2026

Les trois sorties clavier — shell global, chapitre home et laboratoire —
emploient désormais le même contrôle. L'activation transfère le focus vers une
destination `tabIndex=-1`, cadre cette destination et conserve son fragment
d'URL. Le passage ne dépend plus d'un simple défilement visuel : un lecteur
d'écran reprend donc au contenu effectivement demandé.

La sonde intégrée a ensuite été rejouée sur le build de production local, au
niveau de la scène réellement montée :

| Profil | LCP | CLS | INP | tâche longue max. | tâche longue immersive | Canvas | Débordement |
|---|---:|---:|---:|---:|---:|---:|---:|
| mobile 390 × 844 | 700 ms | 0 | non exposé par le navigateur | 150 ms | 0 ms | 1 | 0 px |
| desktop 1280 × 720 | 700 ms | 0 | 184 ms | 150 ms | 0 ms | 1 | 0 px |

Le profil mobile ne fabrique pas une valeur INP lorsque l'API navigateur ne la
publie pas. Les autres seuils mesurables franchissent les gates Phase 19. La
suite web complète, TypeScript et le build Next.js de **47 routes** sont verts ;
la home reste à **121 kB** de premier chargement et le laboratoire à **118 kB**,
le graphe Three.js demeurant différé. Cette preuve reste locale et ne remplace
pas les Core Web Vitals terrain du futur canary.

### Bornage local de la recherche exacte — 3 septembre 2026

L'abstention réelle de la requête `NANKANG Winter Activa SV-3` a été reçue en
production après **32,66 s**. La correction du budget client permet désormais
de recevoir cet état terminal, mais elle a aussi exposé un coût backend inutile
sur les références de modèle explicites.

Le candidat local reconnaît donc les codes avec tiret comme `SV-3`, conserve la
voie lexicale déterministe lorsqu'aucun rayon taxonomique n'est prouvé, pousse
les contraintes de modèle dans la requête SQL avant pagination et produit des
clauses `ILIKE` compatibles avec les index trigrammes PostgreSQL existants.
Cette optimisation reste fail-closed : elle ne crée ni rayon, ni offre, ni
verdict lorsque la preuve manque.

Qualification locale : **2 643 tests backend passent**, **3 sont ignorés** et
l'unique test nécessitant un port loopback passe isolément dans l'environnement
autorisé. Aucun gain de latence production n'est encore revendiqué : cette
version n'est ni publiée ni déployée.

### Passage du laboratoire vers l'expérience FILON réelle — 3 septembre 2026

L'architecture finale distingue trois couches :

1. **Laboratory primitives** — banc d'essai Preview, non indexé, contenant les
   commandes, plans numérotés et instruments de diagnostic ;
2. **Final-grade signature moments** — moteur réutilisable neutre dans
   `components/experience/signature`, sans vocabulaire de laboratoire ;
3. **Real FILON experience** — Home et Produit consomment déjà la scène
   WebGL ; Search, Catalogue, Fashion, Wardrobe, Stylist, Composer et Personal
   Commerce reçoivent uniquement les transformations qui expliquent leur
   vérité métier, avec le DOM comme ledger accessible et fail-closed.

Le laboratoire ne sera pas exposé dans la navigation finale. Son accès est
fail-closed en production et reste disponible en Preview/développement pour
rejouer la qualification. Les labels `LABORATOIRE`, `P19` et `PLAN 01–04`
restent confinés à cette surface.

Le socle conservé pour l'intégration comprend la vraie caméra
`wide → macro → orbit → orthographic`, la continuité physique du produit, la
spatialisation des offres, la mutation de matière selon l'admissibilité, la
lumière et le brouillard. Les transitions DOM/CSS/Framer restent réservées à
la continuité de page, au texte, aux contrôles et aux fallbacks ; elles ne sont
pas présentées comme de la 3D.

La frontière est verrouillée par tests : aucune surface publique ne peut
importer `components/immersive-lab` ni afficher les marqueurs de démonstration.
Les builds locaux confirment le laboratoire en **404 production** et en
**200 Preview**, sans modifier les données ni les flags métier.

### Direction publique daylight — 4 septembre 2026

Le passage du laboratoire au produit ne reprend pas son fond de démonstration.
Le candidat `3e8c08a` installe une grammaire publique commune sur tout le site :
ivoire pour le marché et la lecture, sauge pour la preuve, argile pour la
décision, brun minéral pour la sortie. Les halos sombres, cartes en verre et
checklists préécrites sont retirés des surfaces utilisateur.

La scène de production WebGL suit la même direction. Caméra, profondeur et
transformation matérielle restent réelles, mais s'exécutent dans un espace clair.
La recherche ne montre plus qu'un état d'attente lié à la requête réelle et
attend son terminal SSE avant de conclure. Suite web, TypeScript et build des
47 routes sont verts ; la revue visuelle desktop des huit surfaces critiques
est passée. Publication et Preview restent hors du périmètre de cette passe.
