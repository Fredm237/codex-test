...
| Filiation canary par reçu | `1c9e3b5d7f0a` | Isole chaque observation canary par reçu exact et conserve la filiation du reçu public |
| Périmètre pays V2 | `2d0f4a6c8e1b` | Lie chaque fenêtre V2 à un pays et à une borne RawSource immuable, sans payload brut |

La seule tête attendue est `2d0f4a6c8e1b`. La colonne de devise reste
...
Résultat attendu : une seule tête, `2d0f4a6c8e1b`. Confirmer ensuite hors log
...
La révision courante doit être `2d0f4a6c8e1b (head)` et le check doit afficher
...
Si toutes les tables, contraintes, index et colonnes correspondent déjà
...
4. garder `OBSERVATION_SHADOW_ENABLED=false` et
   ...
   `PERSONAL_COMMERCE_SHADOW_ENABLED=false` et
...
5. obtenir `2d0f4a6c8e1b (head)` avec `alembic current` et un `alembic check`
   sans drift ;
...
Le replay Constraint Engine obéit aux mêmes règles et ne peut lire que des
candidats Hybrid Retrieval déjà persistés :

```bash
CONSTRAINT_ENGINE_SHADOW_ENABLED=true \
python -m app.constraint_engine.replay \
  --evaluated-at <instant-UTC-fixe> \
  --after-run-id 0 \
  --limit 10 \
  --apply
...
`2d0f4a6c8e1b` et garder les shadows désactivés. Les structures d'expansion sont
