...
La seule tête attendue est `1c9e3b5d7f0a`. La colonne de devise reste
...
Résultat attendu : une seule tête, `1c9e3b5d7f0a`. Confirmer ensuite hors log
...
La révision courante doit être `1c9e3b5d7f0a (head)` et le check doit afficher
...
5. obtenir `1c9e3b5d7f0a (head)` avec `alembic current` et un `alembic check`
   sans drift ;
...
`1c9e3b5d7f0a` et garder les shadows désactivés. Les structures d'expansion sont
