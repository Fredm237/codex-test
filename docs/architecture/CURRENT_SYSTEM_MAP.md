...
| `core/observability.py`, `core/local_alerts.py`, `api/middleware.py`, `api/routes/health.py` | Corrélation HTTP et pipeline, latences, readiness, alertes locales et front door pseudonymisée à mémoire bornée | Requêtes HTTP, scope ASGI validé, sorties decision/recommend, ingestion/observation, DB, révision Alembic | Logs échantillonnés, headers, 429, probes, `/health/metrics` et candidat local sans endpoint d'alerte | Mémoire locale au processus, HMAC éphémère, allowlist proxy explicite, aucune requête/IP/identifiant d'offre dans les agrégats | Commit `7cbb81d` isolé : 104 ciblés et backend 1 370/1 370 ; deux revues sans P0/P1/P2 | KEEP + EXTEND | CIDR Railway vérifié, WAF/limite distribuée, ordonnanceur, export, propagation aux dépendances, dashboards et pager testés |
