...
| `.github/workflows` | Workflow Phase 0 multi-surfaces protégé ; rapport externe historique + gate autonome stricte | Push/PR | GitHub checks | Python 3.12, Node 22, pnpm 9.12 | Ruleset `21798272` sans bypass ; ancien run #368 techniquement vert hors ancien gate humain ; workflow autonome local à requalifier | IN PROGRESS | Publier et obtenir les quatre jobs verts sur le nouveau contrat ; aucune attente d'annotateur |
| `quality/`, `filon-backend/quality_lab` | Autonomous Quality Lab v1 + contrat externe v0.5 conservé comme historique | Golden set de régression, holdout adversarial multi-seed, oracles déterministes et signaux multi-source | Rapport `PASS/FAIL`, statuts explicites, cas, limites et identité SHA-256 | Checksum/identité exacts, non-fusion, prix/devise/stock/fraîcheur/budget/abstention ; subjectif provisoire | **571/571** checks, **7/7** tests, 1 conflit correctement `UNRESOLVED`, 0 échec bloquant ; reçu externe toujours `ready=false` | DONE LOCAL | Requalifier à distance ; conserver `NO_EXTERNAL_HUMAN_GROUND_TRUTH` et ne jamais promouvoir un consensus ou un juge modèle en vérité absolue |
...
| Vérité d'évaluation | Quality Lab autonome v1 ; contrat externe v0.5 historique | Oracles déterministes disponibles ; aucune ground truth externe, subjectif non calibré | KEEP AUTONOMOUS + LABEL LIMITS | Holdout adversarial versionné, régressions, multi-source sans consensus absolu et juge modèle auxiliaire audité seulement si nécessaire |
