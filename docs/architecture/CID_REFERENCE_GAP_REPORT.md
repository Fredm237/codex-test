...
- Statut : **assemblage final local qualifié / non publié**
...
## Réévaluation après assemblage dans le vrai FILON

| Axe CID | État du candidat `5820ddf` | Preuve mesurable |
|---|---|---|
| CAMERA | **FERMÉ sur Home et Produit** | Home : progression scroll wide → macro → orbit → orthographique ; Produit : séquence temps réel macro → preuve → stabilisation |
| DEPTH | **FERMÉ sur les moments signature** | caméra perspective, occlusion par fragments, fog, grille et changement de projection réels |
| MATERIAL | **FERMÉ sur les moments signature** | roughness, metalness, opacity et état filaire varient uniquement selon l'admissibilité |
| LIGHT | **FERMÉ sur les moments signature** | rigs directionnel, spot et point suivent la progression causale |
| OBJECT CONTINUITY | **FERMÉ** | le même visuel produit serveur reste monté dans le noyau pendant chaque séquence |
| TRANSITIONS | **FERMÉ pour le gate local** | Home continue sans slide ; Search, Catalogue et Produit conservent le chapitre et l'objet sans cinéma bloquant |
| RHYTHM | **FERMÉ** | intensité 5/5 Home, 4/5 Produit, 2/5 Search/Catalogue, 2→1/5 Décision |
| TYPOGRAPHY / SILENCE | **FERMÉ** | aucune instrumentation visible ; surfaces utilitaires et éditoriales restent DOM/CSS |
| MOBILE | **FERMÉ localement** | 390 × 844, aucune largeur excédentaire, composition portrait et canvas unique seulement sur Produit |
| PERFORMANCE | **FERMÉ localement, terrain en attente** | build 47 routes, Home 121 kB, rendu à la demande ; p75 canary encore requis |

La comparaison finale est capturée localement dans
`FILON_PHASE19_FINAL_DESKTOP.mp4` et `FILON_PHASE19_FINAL_MOBILE.mp4`. Les deux
fichiers montrent le parcours assemblé, à vitesse réelle et dans le bon sens.
Ces fichiers documentent le premier assemblage. Le candidat `5820ddf` remplace
l'objet phare issu du simple tri marchand par une sélection quotidienne de
produits réels technologie/fashion, exige image, écart de prix et au moins deux
marchands, puis borne la texture WebGL par hôte, type et taille. Il reformule
aussi la Home en langage d'achat courant sans modifier les règles de preuve. Le seul écart
restant avec le gate de production est une nouvelle preuve vidéo Preview/canary
sur ce candidat exact, pas une dette de direction créative.

## Décision d'audit

**GO laboratoire confirmé. GO assemblage local sur les surfaces réelles.
VALIDATION PREVIEW requise avant publication. NO-GO production avant canary.**
